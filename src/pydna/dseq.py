...
class Dseq(_Seq):
    ...
    def __init__(
        ...
        if matches := _re.findall(b"[PEXIpexi][QFZJqfzj]|[QFZJqfzj][PEXIpexi]", t):
            raise ValueError(
                f"Molecule is internally split: {', '.join([m.decode('ascii') for m in matches])}"
            )
    ...
    def quick(cls, data: bytes, *args, circular=False, pos=0, **kwargs):
        ...
        obj = cls.__new__(
            cls
        )  # Does not call Bio.Seq.__init__() which has lots of time consuming checks.
    ...
    def full_sequence(self):
        return self.__class__(
            self._data.translate(to_full_sequence), circular=self.circular
        )
    ...
    def five_prime_end(self) -> _Tuple[str, str]:
        ...
        """
        sticky5 = (
            _re.match(b"^[PEXIpexi]*", self._data)
            .group(0)
            .translate(to_watson_table)
            .lower()
        )
        sticky3 = (
            _re.match(b"^[QFZJqfzj]*", self._data)
            .group(0)
            .translate(to_crick_table)
            .lower()
        )

        type_ = "blunt"

        assert (sticky5 or sticky3) or (
            not sticky5 and not sticky3
        ), "End has to be either 5' or 3', not both."
    ...
    def three_prime_end(self) -> _Tuple[str, str]:
        ...
        sticky3 = (
            _re.search(b"[PEXIpexi]*$", self._data)
            .group(0)
            .translate(to_watson_table)
            .lower()
        )
        sticky5 = (
            _re.search(b"[QFZJqfzj]*$", self._data)
            .group(0)
            .translate(to_crick_table)
            .lower()
        )
    ...
    def __getitem__(self, sl: slice) -> "Dseq":
        ...
        if isinstance(sl, slice):
            if self.circular:
                ...
                # A start equal to or larger than stop returns the cut-around slice.
                if sl.start >= sl.stop:
                    return self.__class__(
                        self._data[sl.start :] + self._data[: sl.stop]
                    )
        return self.__class__(super().__getitem__(sl))
    ...
    def shifted(self: DseqType, shift: int) -> DseqType:
        ...
        if not shift:
            ...
            return self.__class__(
                self._data[shift:] + self._data[:shift], circular=True
            )
    ...
    def _fill_in_five_prime(self: DseqType, nucleotides: str) -> str:
        ...
        nucleotides = (
            nucleotides
            if isinstance(nucleotides, bytes)
            else nucleotides.encode("ascii")
        )

        if not nucleotides:
            return _copy.deepcopy(self)

        s = _complement(nucleotides.upper() + nucleotides.lower()).translate(
            to_3tail_table
        )
    ...
    def T4(self, nucleotides: _Union[bytes, str]) -> "Dseq":
        ...
        to_remove = bytes(
            set(b"GATCgatc") - set(nucleotides.upper() + nucleotides.lower())
        )
        strp = self.lstrip(b"QFZJqfzj").rstrip(
            b"PEXIpexi"
        ...
        new = _re.sub(
            b"(?:(?<=[PEXIpexi])|^)([%b]+)" % _complement(to_remove), repl1, strp._data
        )
        ...
        if not _re.search(
            b"([PEXIpexi]|[QFZJqfzj])[GATCgatc]+([PEXIpexi]|[QFZJqfzj])", final
        ):
            import warnings as _warnings
            from pydna import _PydnaWarning

            _warnings.warn(
                "DNA fragment was completely digested by the 3'-5' nuclease activity of T4 DNA pol",
                _PydnaWarning,
            )
            final = b""
    ...
    def seguid(self) -> str:
        ...
        if self.circular:
            cs = _cdseguid(
                self.watson.upper(), self.crick.upper(), alphabet="{DNA-extended},AU"
            )
    ...
    def cut(self: DseqType, *enzymes: EnzymesType) -> _Tuple[DseqType, ...]:
        ...
        cutsite_pairs, shift, stuffer = _get_cutsite_pairs(
            cutsites, self.circular, len(self)
        )

        return tuple(
            self.apply_cut(*cutsite_pair, shift, stuffer)
    ...
    def melt(self, length):
        ...
        result = tuple(
            new.apply_cut(*cutsite_pair, shift, stuffer)
            for cutsite_pair in cutsite_pairs
        )
    ...
    def apply_cut(
        self, left_cut: CutSiteType, right_cut: CutSiteType, shift, stuffer
    ) -> "Dseq":
